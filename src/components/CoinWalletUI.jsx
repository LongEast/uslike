...
export function CoinBalancePill({ balance, onClick, compact = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/58 px-3 py-2 text-sm font-semibold text-stone-700 shadow-sm backdrop-blur-xl transition hover:bg-white/86"
...
export function CheckInCard({ wallet, onOpen, onClaim }) {
  ...
  return (
    <div className="glass-panel rounded-[28px] p-5">
      <button type="button" onClick={onOpen} className="flex w-full items-start gap-3 text-left">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center justify-between gap-2">
            <span className="font-semibold text-stone-800">今日签到</span>
            <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#6b5ee7]">
      ...
      {status.checkedIn ? (
        <div className="mt-4 flex items-center justify-between rounded-2xl bg-[#e6f7f2] px-4 py-3 text-sm font-semibold text-[#2d8c77]">
...
export function CheckInModal({ wallet, onClose, onClaim }) {
  ...
  return (
    <Modal title="每日签到" onClose={onClose} width="max-w-2xl">
      <div className="rounded-[26px] bg-gradient-to-br from-[#eef3ff] to-[#f3ebff] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-[#6b5ee7]">连续相遇，积攒一点心意</p>
        ...
        <div className="mt-5 grid grid-cols-7 gap-2 overflow-x-auto pb-1">
          {CHECK_IN_REWARDS.map((reward, index) => {
            ...
            return (
              <div
                key={day}
                className={`min-w-[62px] rounded-2xl border px-2 py-3 text-center ${
                  claimed
                    ? "border-[#a8dfd1]/70 bg-[#e6f7f2] text-[#2d8c77]"
                    : today
                      ? "border-[#8b82e8]/55 bg-white text-[#6b5ee7] shadow-sm"
                      : "border-white/80 bg-white/48 text-stone-400"
                }`}
              >
      ...
      {status.checkedIn ? (
        <div className="mt-5 rounded-2xl bg-[#e6f7f2] px-5 py-4 text-center font-semibold text-[#2d8c77]">
